...
public abstract class AresSerialPort : IAresSerialPort
{
  ...
  /// </param>
  protected internal AresSerialPort(SerialPortConnectionInfo connectionInfo, string portName, TimeSpan? sendBuffer = null)
  {
  ...
  private void ProcessBufferCore()
  {
    ...
    lock ( _multiResponseQueue )
    {
      if (_buffer.Any())
      {
        ...
        // Trace.WriteLine($"~~ Processed and published: \"{values.Response}:{Encoding.ASCII.GetString(values.DataToRemove?.ToArray()).ReplaceLineEndings("")}\" ~~");
      }
    }
  ...
  protected void AddDataReceived(byte[] dataReceived)
  {
    lock ( _bufferLock )
    {
      ...
      // Trace.WriteLine($"~~ Added: \"{Encoding.ASCII.GetString(dataReceived).ReplaceLineEndings("")}\" ~~");
    }
