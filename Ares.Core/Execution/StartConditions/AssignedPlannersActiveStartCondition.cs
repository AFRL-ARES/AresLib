...
public class AssignedPlannersActiveStartCondition : IStartCondition
{
  ...
  public StartConditionResult? CanStart()
  {
    ...
    foreach(var adapter in assignedPlanners)
    {
      ...
      if(planner is null)
        return new StartConditionResult(false, $"Assigned Planner Adapter {adapter.AdapterName} could not be found in the core.");

      if(planner.Status.PlannerState != PlannerState.Active)
        return new StartConditionResult(false, $"Assigned Planner Adapter {adapter.AdapterName} is not connected to ARES.");
    }
